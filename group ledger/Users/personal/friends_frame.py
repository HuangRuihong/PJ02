...
class FriendsFrame(ctk.CTkFrame):
    ...
    def open_repay_dialog(self, target, amount):
        ...
        m_var = ctk.StringVar(value="現金")
        for m in ["現金交付", "轉帳 / LINE Pay", "其他抵銷"]: ctk.CTkRadioButton(dialog, text=m, variable=m_var, value=m).pack(pady=2)
        
        def submit():
            ids = [i['tx_id'] for i in self.system.get_personal_debts(self.current_user)[0] if i['creditor'] == target]
            if self.system.request_settlement(self.current_user, target, amount, m_var.get(), ids):
                mbox.showinfo("成功", "申請已送出！", parent=self.winfo_toplevel()); self.winfo_toplevel().refresh_ui() 
            dialog.destroy()
