...
class PersonalService(BaseService):
    ...
    def request_settlement(self, debtor_id, creditor_id, amount, method, tx_ids):
        ...
        try:
            with self._get_connection() as conn:
                c, s_id = conn.cursor(), f"req_{uuid.uuid4().hex[:8]}"
                c.execute("INSERT INTO transactions (transaction_id, group_id, payer_id, amount, status, type, description, location, timestamp, category) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                ...
                c.execute("INSERT INTO transaction_participants (transaction_id, user_id, owed_amount, status) VALUES (?, ?, ?, ?)", (s_id, creditor_id, amount, TransactionStatus.PENDING.name))
