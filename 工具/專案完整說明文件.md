...
利用 `refresh_ui` 的依賴聯動確保資料不撕裂，並建立 `run_scheduler` 背景執行緒不阻塞 UI。
#### 2. `analysis/` ：數據分析與圖表繪製
利用 `update_chart` 透過 Matplotlib 動態鑲嵌圓餅圖表。
#### 3. `components / dialogs.py` ：解耦彈窗組件
採用 Callback 模式，保證 UI 與底層商業邏輯剝離。
#### 4. `group_frame.py` ：群組空間面板
實作 `handle_settle` 銜接「一鍵結清(Greedy)」與介面選點。並擁有 `handle_notify` 發送預警能力。
#### 5. `personal_frame.py` ：個人主控儀表板
透過 `load_real_data` 將跨群組收支統整，並支援 `do_confirm` 單一確認銷帳。
#### 6. `friends_frame.py` ：單向好友列表
透過 `refresh` 會依重要度與金額自動將亟需還款的好友清單置頂。
