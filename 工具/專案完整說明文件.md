...
4. **全域唯一標識符 (UUID Compliance)**
   - 核心 ID 生成捨棄不穩定的時間戳，全面採用 UUID v4 技術，確保在高頻記帳與多人併發同步時具備 100% 的碰撞防護。
...
| :--- | :--- | :--- |
| **精準交易均分** | 採 `基數 + 餘數遞增` 邏輯，確保分帳總和與原始金額 100% 吻合，無 1 元誤差。 | [group_service.py:L105-108](group_service.py#L105-L108) |
| **債務簡化與抵銷** | 使用 **貪婪演算法 (Greedy)**，藉由排序債務與債權清單，極小化結算時的轉帳總次數。 | [group_service.py:L226-243](group_service.py#L226-L243) |
| **狀態機自動躍遷** | 實作多參與者狀態優先級權重判定，包含「一票否決 (Rejected)」與「全員確認」邏輯。 | [group_service.py:L149-178](group_service.py#L149-L178) |
| **淨餘額計算** | 動態加總所有確認交易中個人的 `應收額 - 應付額`，得出即時帳務淨值。 | [group_service.py:L198-216](group_service.py#L198-216) |
| **剩餘預算監控** | 公式：`Remaining = Budget - SUM(Active Expenses)`，提供群組消費預警。 | [group_service.py:L89](group_service.py#L89) |
| **隨機邀群碼** | 結合 `random.choices` 與資料庫唯一性遞迴檢查，生成 6 位不重複英數邀請碼。 | [group_service.py:L14-20](group_service.py#L14-L20) |
| **個人全域債務對沖** | 跨群組針對特定好友進行全域債務加總，方便個人私下結帳參考。 | [personal_service.py:L131-137](personal_service.py#L131-L137) |

### 5.5 核心程式碼實作片段 (Core Code Implementation)
以下展示系統中 7 項核心功能的具體 Python 實作邏輯：

#### 5.5.1 精準交易均分邏輯 (處理除不盡的餘數)
```python
# Location: group_service.py:L105-108
base = amount_twd // count
rem = amount_twd % count
for i, uid in enumerate(participants):
    # 前 rem 位參與者額外分配 1 元以湊齊總額
    splits[uid] = base + (1 if i < rem else 0)
```

#### 5.5.2 債務簡化與抵銷演算法 (Greedy)
```python
# Location: group_service.py:L235-243
...
#### 5.5.3 交易狀態機優先級判定 (一票否決制)
```python
# Location: group_service.py:L162-175
...
#### 5.5.4 淨餘額計算邏輯 (Net Balance Calculation)
```python
# Location: group_service.py:L212-214
for user_id, owed_amt, payer_id in rows:
    # 應收增加 payer 的餘額，應付減少參與者的餘額
...
#### 5.5.5 預算剩餘計算公式
```python
# Location: group_service.py:L86-90
return {
    "budget": budget,
    "spent": spent,
...
#### 5.5.6 隨機邀群碼生成邏輯 (帶唯一性檢查)
```python
# Location: group_service.py:L14-20
...
#### 5.5.7 個人全域債務對沖邏輯
```python
# Location: personal_service.py:L131-137
for p in payables:
    creditor = p["creditor"]
